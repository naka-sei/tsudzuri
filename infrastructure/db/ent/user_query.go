...
// QueryInvitedPages chains the current query on the "invited_pages" edge.
func (_q *UserQuery) QueryInvitedPages() *PageQuery {
	query := (&PageClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withInvitedPages: _q.withInvitedPages.Clone(),
...
// WithInvitedPages tells the query-builder to eager-load the nodes that are connected to
// the "invited_pages" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithInvitedPages(opts ...func(*PageQuery)) *UserQuery {
	query := (&PageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInvitedPages = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withInvitedPages; query != nil {
		if err := _q.loadInvitedPages(ctx, query, nodes,
			func(n *User) { n.Edges.InvitedPages = []*Page{} },
			func(n *User, e *Page) { n.Edges.InvitedPages = append(n.Edges.InvitedPages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadCreatedPages(ctx context.Context, query *PageQuery, nodes []*User, init func(*User), assign func(*User, *Page)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(page.FieldCreatorID)
	}
...
func (_q *UserQuery) loadInvitedPages(ctx context.Context, query *PageQuery, nodes []*User, init func(*User), assign func(*User, *Page)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.InvitedPagesTable)
		joinT.Schema(_q.schemaConfig.PageInvitedUsers)
		s.Join(joinT).On(s.C(page.FieldID), joinT.C(user.InvitedPagesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.InvitedPagesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.InvitedPagesPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Page](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "invited_pages" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
